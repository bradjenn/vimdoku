...
export const createRoom = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const turnLives =
      battleKind === 'turns' ? cleanTurnLives(args.turnLives) : undefined
    const turnSeconds =
      battleKind === 'turns' ? cleanTurnSeconds(args.turnSeconds) : undefined
    ...
    const titleParts = [
      puzzleSize,
      playMode,
      variantId === 'classic' ? null : variantId,
      difficulty,
    ]
      .filter(Boolean)
      .join(' / ')
...
export const heartbeat = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const nextPresence = {
      ...
      lives:
        existing?.lives ??
        (room.battleKind === 'turns' ? (room.turnLives ?? 3) : undefined),
    ...
    const nextRoomPresence = roomPresence.some((row) => row.anonId === anonId)
      ? roomPresence.map((row) =>
          row.anonId === anonId ? { ...row, ...nextPresence, anonId } : row,
        )
      : [...roomPresence, { ...nextPresence, anonId, roomId }]
    const readyRacePlayers = nextRoomPresence.filter(
      (row) =>
        row.status === 'ready' ||
        row.status === 'solving' ||
        row.status === 'finished',
    ...
    const nextStatus =
      args.status === 'finished' && room.status !== 'finished'
        ? 'finished'
        : room.status === 'waiting' &&
            (raceCountdownDone ||
...
export const submitTurn = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const room = await ctx.db
      ...
      .unique()
    if (!room) throw new Error('Live battle room not found.')
    if (room.battleKind !== 'turns')
      return { ok: false, message: 'Room is not turn-based.' }
    if (room.status === 'finished')
      return { ok: false, message: 'Battle is finished.' }
    ...
    if (room.turnAnonId !== anonId)
      return { ok: false, message: 'Not your turn.' }

    const now = Date.now()
    const timedOut =
      typeof room.turnEndsAt === 'number' && room.turnEndsAt <= now
    const lifeLoss = timedOut || !args.correct ? 1 : 0
    const nextLives = Math.max(
      0,
      (existing.lives ?? room.turnLives ?? 3) - lifeLoss,
    )
    const nowIso = new Date().toISOString()
    ...
    if (winner) {
      ...
      return {
        ok: true,
        message: winner === anonId ? 'You won.' : 'Battle finished.',
      }
    }
...
export const claimTimeout = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const nextLives = Math.max(0, (current.lives ?? room.turnLives ?? 3) - 1)
...
function cleanTurnLives(value: number | undefined) {
